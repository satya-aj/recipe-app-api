...
class PublicUserAPiTests(TestCase):
    ...
    def test_user_exists(self):
        "Test creating user that already existing"
        payload = {
            'email': 'test@test.com',
            'password': 'test123',
            'name': 'Test'
    ...
    def test_create_token_invalid_creadentials(self):
        ...
        create_user(email='test@test.com', password='testpas')
        payload = {'email': 'test@payload.com', 'password': 'wrong'}
        res = self.client.post(TOKEN_URL, payload)
...
class PrivateUserAPITest:
    "Test API requests the requires auth"
    def setUp(self):
    ...
    def test_retrive_profile_success(self):
        "Test retriveing profile for logged user"
        res = self.client.get(ME_URL)
        self.assertEqual(res.status, status.HTTP_200_SOK)
    ...
    def test_post_me_not_allowed(self):
        ...
        self.assertEquel(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
